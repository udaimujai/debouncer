...
static void debouncer_uppdate_btn(uint8_t index)
{
    if(btn_pins_STATE[index] == DEBOUNCER_STATE_FALLING)
    {
        btn_pins_STATE[index] = DEBOUNCER_STATE_LOW;
    }
    else if (btn_pins_STATE[index] == DEBOUNCER_STATE_RISING) {
    ...
    if(pin_state == btn_pins_STATE[index])
        ...
        if(btn_counter[index] == DEBOUNCER_NO_RIPPLE)
        {
            ...
            if(btn_pins_STATE[index] == DEBOUNCER_STATE_LOW)
            {
                ...
                btn_pins_STATE[index] = DEBOUNCER_STATE_RISING;
            }
            else{
                btn_pins_STATE[index] = DEBOUNCER_STATE_FALLING; 
            }
        }
    }
}
